...
if (!empty($keyword)) {
    $where .= ' AND (title LIKE :keyword OR description LIKE :keyword OR content LIKE :keyword OR client LIKE :keyword)';
    $params[':keyword'] = "%{$keyword}%";
}

// 获取总记录数
$total_stmt = $pdo->prepare("SELECT COUNT(*) as total FROM cases WHERE {$where}");
...
// 获取推荐案例
$recommend_cases = $pdo->query("SELECT id, title, cover_image 
                               FROM cases 
...
<head>
    ...
    <title>成功案例 - <?php echo $site_config['name']; ?></title>
    <meta name="description" content="福州佳势展览有限公司成功案例展示，包括展览展陈、标牌工程、活动布置等多个领域的优秀案例">
    <meta name="keywords" content="展览成功案例,标牌工程案例,活动布置案例,佳势展览案例">
    <!-- 引入外部资源 -->
    ...
    <style type="text/tailwindcss">
        @layer utilities {
            ...
            .case-card-overlay {
                @apply absolute inset-0 bg-dark/60 opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-center justify-center;
            }
        }
    </style>
</head>
<body class="font-sans text-secondary bg-light">
    ...
    <section class="bg-primary/10 py-24 md:py-32">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center">
                <h1 class="text-[clamp(2rem,4vw,3rem)] font-bold text-secondary mb-4">成功案例</h1>
                <p class="text-gray-600 max-w-2xl mx-auto">
                    展示我们在展览展陈、标牌工程、活动布置等领域的成功案例，见证我们的专业实力
                </p>
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="text-primary">成功案例</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
    ...
    <section class="section-padding bg-white">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <!-- 搜索和筛选 -->
            <div class="bg-accent rounded-xl p-6 mb-10">
                <form method="get" action="cases.php" class="flex flex-col md:flex-row gap-4">
                    <div class="flex-grow">
                        <div class="relative">
                            <input type="text" name="keyword" placeholder="请输入关键词搜索案例..." 
            ...
            <div class="mb-10 overflow-x-auto pb-2">
                <div class="flex space-x-2 min-w-max">
                    <a href="cases.php" class="px-5 py-2 rounded-full text-sm font-medium transition-colors 
                        <?php echo $category_id == 0 ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-primary/10'; ?>">
                        全部案例
                    </a>
                    <?php foreach ($categories as $category): ?>
                    <?php 
                    // 获取每个分类的案例数量
                    $cat_count_stmt = $pdo->prepare("SELECT COUNT(*) as count FROM cases WHERE category_id = :id AND status = 1");
                    $cat_count_stmt->execute([':id' => $category['id']]);
                    $cat_count = $cat_count_stmt->fetch()['count'];
                    ?>
                    <a href="cases.php?category_id=<?php echo $category['id']; ?>" 
                       class="px-5 py-2 rounded-full text-sm font-medium transition-colors 
                           <?php echo $category_id == $category['id'] ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-primary/10'; ?>">
                        <?php echo $category['name']; ?> <span class="ml-1 opacity-80">(<?php echo $cat_count; ?>)</span>
                    </a>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <!-- 推荐案例 -->
            <?php if (count($recommend_cases) > 0 && $category_id == 0 && empty($keyword)): ?>
            <div class="mb-16">
                <h2 class="section-title flex items-center">
                    <i class="fa fa-star text-primary mr-3"></i> 推荐案例
                </h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                    <?php foreach ($recommend_cases as $case): ?>
                    <a href="case-detail.php?id=<?php echo $case['id']; ?>" class="block group">
                        <div class="relative rounded-xl overflow-hidden card-shadow">
                            ...
                            <div class="case-card-overlay">
                                <span class="bg-primary text-white px-4 py-2 rounded-full text-sm font-medium transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                                    查看详情 <i class="fa fa-angle-right ml-1"></i>
                                </span>
                            </div>
                        </div>
                        <h3 class="text-lg font-medium text-secondary mt-4 group-hover:text-primary transition-colors">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <a href="case-detail.php?id=<?php echo $case['id']; ?>" class="block group">
                    ...
                    <h3 class="text-base font-medium text-secondary mt-4 group-hover:text-primary transition-colors line-clamp-2">
                        <?php echo htmlspecialchars($case['title']); ?>
                    </h3>
                    <p class="text-gray-500 text-sm mt-1">客户：<?php echo htmlspecialchars($case['client']); ?></p>
                </a>
                <?php endforeach; ?>
            </div>
            ...
            <div class="text-center py-16 bg-accent rounded-xl">
                ...
                <h3 class="text-xl font-medium text-gray-500 mb-2">未找到相关案例</h3>
    ...
    <section class="section-padding bg-primary/10">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="max-w-4xl mx-auto text-center">
                <h2 class="section-title">需要定制您的专属方案？</h2>
                <p class="text-gray-600 max-w-2xl mx-auto mb-8">
                    无论您有任何展览展示需求，我们都能为您提供专业的解决方案，欢迎联系我们获取详细报价
                </p>
